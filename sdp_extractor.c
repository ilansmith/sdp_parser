		...
		int *packet_size)
{
	...
	pixles_total = fmtp_params->width * fmtp_params->height;
	...
	if (fmtp_params->pm == PM_2110BPM) {
		...
		pixles_per_packet = pg_per_packet * pgi.coverage;

		__npackets = (pixles_total + (pixles_per_packet - 1)) /
			pixles_per_packet;
		...
		*npackets = __npackets;
		...
		pg_num = pixles_total / pgi.coverage;
